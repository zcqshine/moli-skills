...
现在 AI 开发代码快，但是人工测试起来头大，纯 API 测试可以用测试用例覆盖，但是人工用网页做端到端（E2E）测试就效率低了：改一行代码，把所有表单再填一遍、把所有校验再试一遍。想上自动化，CI 小机器上 `Chrome Headless` 又动辄 `700MB+`，跑几个浏览器就 `OOM`。

周末的时候无意间刷到了这个用 `Rust` 写的无头浏览器，而且自带了 3 个 `skill`，又看了下介绍，哇靠，这玩意用来做网页自动化测试不是挺好的吗，顺手就写了个 `moli-e2e-test` 测试用的`skill` ，写完代码，直接调用这个 `skill` 做一次网页测试，测试中发现的问题 `agent` 会调用大模型直接修正，然后再回归测试，一套组合下来，写好的代码再交给测试去测试就会少很多问题了。

现在一条命令跑完全部功能与边界用例，出一份 HTML 报告。
